      ...
      <div className="flex flex-col md:flex-row items-center gap-6 md:gap-6">
        ...
        >
          <div className="space-y-2">
            ...
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.2em]">Ref: #{order.id.slice(0, 5).toUpperCase()}</p>
          </div>

          <div className="flex flex-wrap items-center justify-center md:justify-start gap-2.5">
            {order.items.slice(0, 2).map((item, idx) => (
              <div key={idx} className="flex items-center gap-2 bg-slate-50/80 px-3 py-1.5 rounded-xl border border-slate-100/50">
                <span className="text-emerald-600 font-bold text-xs">{item.quantity}x</span>
                <span className="text-slate-500 text-[11px] font-semibold truncate max-w-[120px]">{item.dish.name}</span>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        >
          <div className="text-center md:text-right">
            <p className="text-2xl md:text-2xl font-bold text-slate-700 tracking-tight leading-none">
              <span className="text-slate-600 font-medium">Rs.</span> {order.totalAmount.toLocaleString()}
            </p>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1.5">{order.payment?.method || 'CASH'}</p>
          </div>
          ...
          <div className="flex items-center gap-2.5 bg-slate-50 px-4 py-1.5 rounded-full border border-slate-100">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-slate-600">
              {order.status === 'PENDING' ? 'Processing' :
                order.status === 'PREPARING' ? 'Kitchen hand-crafting' :
                  order.status === 'READY' ? 'Ready for handover' : 'Completed'}
            </span>
          </div>
        </motion.div>
      </div>
