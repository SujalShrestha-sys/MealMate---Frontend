  ...
  return (
    <div className={`w-full ${isMobile ? 'py-6 px-2' : 'py-4 px-1'}`}>
      <div className="relative flex items-center justify-between">
        ...
        {/* Background Track - Ultra Minimal */}
        <div className={`absolute left-0 top-1/2 w-full -translate-y-1/2 bg-slate-100/60 rounded-full ${isMobile ? 'h-[2.5px]' : 'h-[2.5px]'}`} />
          ...
          return (
            <div key={step.id} className="relative flex flex-col items-center">
              <motion.div
                ...
                className={`flex items-center justify-center rounded-full border transition-all duration-700 z-10 ${isMobile ? 'h-2.5 w-2.5' : 'h-3 w-3'
                  } ${isCompleted ? 'bg-emerald-500 border-emerald-500' : ''
                  }`}
              />

              <div className={`absolute flex flex-col items-center ${isMobile ? '-top-9' : '-top-10'}`}>
                <span className={`font-bold uppercase tracking-[0.14em] whitespace-nowrap transition-all duration-500 ${isMobile ? 'text-[11px]' : 'text-[11px]'
                  } ${isActive ? 'text-emerald-600/90' : isCompleted ? 'text-slate-500' : 'text-slate-300'
                  }`}>
                  {step.label}
                </span>
