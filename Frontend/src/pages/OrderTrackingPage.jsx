        ...
        "items": [
          {
            ...
            "dish": {
              "name": "Avocado Toast",
              "imageUrl": "https://images.unsplash.com/photo-1525351484163-7529414344d8?ixlib=rb-4.0.3&auto=format&fit=crop&w=1400&q=80",
            }
          }
        ],
        "payment": {
        ...
        "items": [
          {
            ...
            "dish": {
              "name": "Quinoa Bowl",
              "imageUrl": "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?ixlib=rb-4.0.3&auto=format&fit=crop&w=1350&q=80",
            }
          }
        ],
        "payment": {
                ...
                >
                  {activeOrders.length > 0 ? (
                    ...
                    >
                      ...
                      <p className="text-slate-500 font-medium max-w-xs text-sm">Your dining journey starts here. Place an order to begin.</p>
                    </motion.div>
                  )}
                </motion.div>
              ) : (
                ...
                >
                  {pastOrders.length > 0 ? (
                    pastOrders.map(order => (
                      <div key={order.id} className="bg-white p-4 rounded-2xl border border-slate-100 flex justify-between items-center group hover:border-emerald-100/50 hover:bg-slate-50/50 transition-all duration-500">
                         <div className="flex items-center gap-4">
                           <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-100 overflow-hidden shadow-sm">
                             <img src={order.items[0].dish.imageUrl} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="" />
                           </div>
                           <div className="space-y-0.5">
                             <h4 className="font-bold text-slate-900 group-hover:text-emerald-700 transition-colors text-base tracking-tight">{order.items[0].dish.name}</h4>
                         ...
                         <div className="text-right space-y-1">
                           <p className="font-bold text-slate-900 text-lg tracking-tight">Rs. {order.totalAmount.toLocaleString()}</p>
                           <span className="text-[9px] font-black text-emerald-600/60 uppercase tracking-widest bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-100/50">Archived</span>
                         </div>
                      </div>
                    ))
                  ) : (
                    <div className="py-24 flex flex-col items-center text-center">
                       ...
                       <h3 className="text-xl font-bold text-slate-900 mb-2">Empty Archive</h3>
                       <p className="text-slate-500 font-medium max-w-xs text-sm">Your culinary history will be preserved here.</p>
                    </div>
                  )}
                </motion.div>
              )}
